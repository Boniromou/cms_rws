class FrontMoneyController < ApplicationController
  ...
  def do_search
    begin
      raise FrontMoneyHelper::NoResultException.new "shift not found" if (params[:select_casino] and is_date?(params[:accounting_date]) and (Time.parse(params[:accounting_date]) > current_accounting_date_by_casino_id(params[:select_casino]).accounting_date))
