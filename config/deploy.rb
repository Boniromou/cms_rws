...
set(:cronjobs) {
  ...
  ["0 0 6 * * * #{deploy_to}/current/cronjob/roll_shift.sh #{stage} >> #{deploy_to}/current/log/roll_shift#{stage}.log 2>&1"]
  ["0 0 14 * * * #{deploy_to}/current/cronjob/roll_shift.sh #{stage} >> #{deploy_to}/current/log/roll_shift#{stage}.log 2>&1"]
  ["0 0 22 * * * #{deploy_to}/current/cronjob/roll_shift.sh #{stage} >> #{deploy_to}/current/log/roll_shift#{stage}.log 2>&1"]
}
