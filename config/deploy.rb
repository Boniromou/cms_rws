set :stages, %w(integration0 staging0 production0 sandbox0 sandbox2 smartocean2 mockup0 sandbox3 production3 sandbox ias_prd)
...
#   "* * * * * #{deploy_to}/current/cronjob/approve_transaction.sh  #{stage} >> #{deploy_to}/current/log/approve_transaction_#{stage}.log 2>&1",
   ...
   "0 * * * * #{deploy_to}/current/cronjob/add_shift_id.sh #{stage} >> #{deploy_to}/current/log/add_shift_id_#{stage}.log 2>&1"]
}
