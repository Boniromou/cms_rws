class InsertInitData < ActiveRecord::Migration
  def up
    ...
    execute "INSERT INTO accounting_dates(id,accounting_date, created_at, updated_at) values(1,'#{Time.now.utc}', '#{Time.now.utc}', '#{Time.now.utc}');"
    ...
    Shift.delete_all
    execute "INSERT INTO shifts(id,shift_type_id,accounting_date_id, created_at, updated_at) values(1, 1, 1, '#{Time.now.utc}', '#{Time.now.utc}');"
  end
