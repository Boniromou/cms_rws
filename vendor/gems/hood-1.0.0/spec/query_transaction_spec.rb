...
describe Cashier, "query transactions" do
  ...
  context "query_wallet_transactions" do
    it "MissingRequiredParameters" do
      ib = {:property_id=>1000}
      ob = query_wallet_transactions(ib)
    ...
    it "OK" do
      ...
      allow(Time).to receive(:now).and_return(now)
      WalletTransaction.create(:ref_trans_id=>'wt1',:amt=>12345,:aasm_state=>'completed',:before_balance=>0,:after_balance=>12345,:trans_type=>'deposit',:property_id=>1000,:player_id=>100,:created_at=>'2014-09-09 12:00:00',:trans_date=>'2014-09-09 12:25:00')
      WalletTransaction.create(:ref_trans_id=>'wt2',:amt=>54321,:aasm_state=>'completed',:before_balance=>54321,:after_balance=>0,:trans_type=>'withdraw',:property_id=>1000,:player_id=>100,:created_at=>'2014-09-09 12:30:00',:trans_date=>'2014-09-09 12:25:00')
      WalletTransaction.create(:ref_trans_id=>'wt3',:amt=>12345,:aasm_state=>'completed',:before_balance=>0,:after_balance=>12345,:trans_type=>'deposit',:property_id=>1000,:player_id=>100,:created_at=>'2014-09-10 13:00:00',:trans_date=>'2014-09-10 12:25:00')
      ...
      expect(wt1[:trans_date]).to eq '2014-09-09 21:25:00'
  ...
  context "query_round_transactions" do
    it "OK" do
      ...
      RoundTransaction.create(:ref_trans_id=>'t1',:bet_amt=>12345,:aasm_state=>'completed',:before_balance=>12345,:after_balance=>0,:trans_type=>'bet',:property_id=>1000,:player_id=>100,:created_at=>'2014-09-09 12:00:00',:trans_date=>'2014-09-09 12:25:00',:round_id=>123,:external_game_id=>456)
      RoundTransaction.create(:ref_trans_id=>'t2',:payout_amt=>10050,:win_amt=>-9000,:aasm_state=>'completed',:before_balance=>0,:after_balance=>10050,:trans_type=>'result',:property_id=>1000,:player_id=>100,:created_at=>'2014-09-09 12:10:00',:jc_jp_con_amt=>1234.567,:jc_jp_win_amt=>1234567,:pc_jp_con_amt=>123456,:pc_jp_win_amt=>123456789,:jp_win_id=>'winid',:jp_win_lev=>1,:jp_direct_pay=>1,:trans_date=>'2014-09-09 12:25:10',:round_id=>123,:external_game_id=>456)
      RoundTransaction.create(:ref_trans_id=>'t3',:payout_amt=>10050,:win_amt=>-9000,:aasm_state=>'completed',:before_balance=>0,:after_balance=>10050,:trans_type=>'result',:property_id=>1000,:player_id=>100,:created_at=>'2014-09-09 12:10:00',:jc_jp_con_amt=>1234.567,:pc_jp_con_amt=>123456,:trans_date=>'2014-09-09 12:25:10',:round_id=>123,:external_game_id=>456)
      ib = {:property_id=>1000,:from_time=>'2014-09-09 21:00:00',:to_time=>'2014-09-09 22:00:00'}
      ob = query_round_transactions(ib)
      expect(ob[:error_code]).to eq('OK')
      transactions = ob[:transactions]
      expect(transactions.length).to eq 3
      t1 = transactions.find {|t| t[:ref_trans_id]=='t1'}
      expect(t1[:trans_type]).to eq 'bet'
      expect(t1[:status]).to eq 'completed'
      expect(t1[:bet_amt]).to eq 123.45
      expect(t1[:before_balance]).to eq 123.45
      expect(t1[:after_balance]).to eq 0
      expect(t1[:login_name]).to eq 'player1'
      expect(t1[:trans_date]).to eq '2014-09-09 21:25:00'
      expect(t1[:game_id]).to eq 456

      t2 = transactions.find {|t| t[:ref_trans_id]=='t2'}
      expect(t2[:trans_type]).to eq 'result'
